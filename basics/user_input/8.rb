...
back to a string without loss of information, false otherwise. It's not a perfect solution 
in that some inputs that are otherwise valid (such as 003) will fail, but it 
...
loop do 
  ...
  if valid_number?(numerator) && valid_number?(denominator) 
    if denominator.to_i != 0
      ...
      puts "Invalid input. Denominator cannot be zero."
    end
  else
...
puts "It's time for division."
puts "#{numerator} / #{denominator} is #{result}. Division!"


#alternate solution: break the inputs into separate loops
